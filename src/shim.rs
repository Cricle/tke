...
pub(crate) fn write_all_resilient<W: Write>(
    writer: &mut W,
    mut bytes: &[u8],
) -> Result<(), AppError> {
...
pub(crate) fn run_agent_command(
    ...
    if should_bridge_agent_with_pty(name, &stdin_payload) {
        let tracker = InteractiveTracker::start_for_agent(name);
        let code = crate::pty::passthrough_with_native_pty(real_cmd, args, Some(envs), true)?;
        if let Some(tracker) = tracker {
            tracker.finish(config)?;
        }
        return Ok(code);
...
pub fn run_tty_wrapped(
    ...
    let tracker = InteractiveTracker::start_for_agent(name);
    let code =
        crate::pty::passthrough_with_native_pty(&target_cmd, &target_args, Some(envs), true)?;
    if let Some(tracker) = tracker {
        tracker.finish(config)?;
    }
    Ok(code)
